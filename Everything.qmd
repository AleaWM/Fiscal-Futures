...
Ideally the money going in and out of the funds used for refunds would be approximately equal.

```{r}
#| label: fig-tax-refunds-oldcode
#| fig-cap:  Tax Refunds with Old Code
#| eval: false
...
tax_refunds_rev <- rev_temp %>% 
  filter(str_detect(fund_name, "REFUND") |                   # search string for refund, just in case
...
tax_refunds_exp <- exp_temp %>% 
  filter(str_detect(fund_name, "REFUND")
           | fund %in% c("0121", "0278", "0378", "0380", "0671"))

tax_refunds_exp %>% group_by(fy, fund, fund_name) %>% summarize(refunds_exp = sum(expenditure)) %>% pivot_wider(id_cols = c(fund, fund_name), names_from = fy, values_from = refunds_exp)
```
...
tax_refunds_exp %>% filter(agency != "799") %>% group_by(fy, appr_org, org_name, fund, fund_name) %>% summarize(refunds_exp = sum(expenditure)) %>% pivot_wider(id_cols = c(appr_org, org_name, fund, fund_name), names_from = fy, values_from = refunds_exp)

tax_refunds_exp %>% filter(agency != "799") %>% group_by(fy, object, fund, fund_name) %>% summarize(refunds_exp = sum(expenditure)) %>% pivot_wider(id_cols = c(object, fund, fund_name), names_from = fy, values_from = refunds_exp) %>% arrange(object)
...
After exporting the expenditures and revenues that were related to Refund funds (identified by searching for string containing "refund" and then also identifying the fund number because, in early years, data requested did not have fund labels associated with fund numbers. Yes, you could do just the fund numbers in the command, but if anything changes, looking for strings that contain "refund" would hopefully alert the coder that something new existed which would otherwise be more dificult if only using fund numbers).

```{r}
#| label: fig-tax-refunds-newway
#| fig-cap:  Tax Refunds - New Way
...
tax_refund_long <- exp_temp %>%           # fund != "0401" # removes State Trust Funds
  ...
  # keeps these objects which represent revenue, insurance, treasurer,and financial and professional reg tax refunds
  mutate(refund = case_when(
    fund=="0278" & object == "9910" ~ "02", # for income tax refund
    fund=="0278" & object == "9900" ~ "03", # for corporate income tax refund
  
    fund == "0380" ~ "03", # tax administration and enforcement and tax operations become corporate income tax
  
    fund == "0121" ~ "21",                # inheritance tax and estate tax refund appropriation
  
    fund == "0946" ~ "09",                # motor fuel tax refunds & aviation fuel sales tax refund
  
    obj_seq_type == "99250055" ~ "06",    # sales tax refund, 
  
   object == "9939" ~ "06", # rent purchase agreement tax refund, some years
...
We also do not want to include tax amnesty revenue sources that went into refund funds.

```{r}
rev_temp <- rev_temp %>% filter(source %in% c("1905", "1906", "1923", "2138")     # amnesty rev sources
)
```
...
annotation_nums <- data.frame(
  x = c(2023, 2023, 2023),
  y = c(95, 120, 10),  
  label = c("Exp = $110.6","Rev = $111.7", "$1.2")
)
...
fiscal_gap1
#ggsave(fiscal_gap1, file = "./paper-figures/Figure7a-WithCURE.eps")
...
#ggsave(fiscal_gap_droppedCURE, file = "./paper-figures/Figure7b-WithoutCURE.eps")
```
