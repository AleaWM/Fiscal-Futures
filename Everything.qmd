...
```{r}
#| label: year-totals-table-withgap
...
kable_classic() %>%   add_footnote(c("Methodology has changed slightly since FY21 so totals may differ from past publications", "All changes documented in ___ document.","Values include State CURE dollars"))
```
...
```{r}
#| label: bargraphs-largest-expandrev-cats
#| fig-cap: Largest Expenditures and Revenue Sources for FY2022
...
```{r}
#| label: dotplots
#| fig-cap: Change in Revenue Sources and Expenditure Categories, FY21 to FY22
#| fig-subcap: 
...
```{r}
#| label: ownsource-and-fed-revenue
...
```{r}
#| label: good-exp-CAGR-withTotals
...
```{r}
#| label: good-rev-CAGR-withTotals
...
```{r}
#| label: yearlychange-table-expenditures
...
```{r}
#| label: yearlychange-table-revenues
#| tbl-cap: Yearly Change in Revenues
...
```{r }
#| label: top10-exp-CAGRs
#| tbl-cap: Top 13 Expenditures
...
```{r}
#| label: totals-without-FederalStimulus
...
```{r}
#| label: withandwithout-stateCURE
...
```{r }
#| label: large-cat-CAGRs-withoutCURE
...
```{r}
#| label: top10-rev-droppedCURE-yearlychange
