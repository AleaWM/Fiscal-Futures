...
ff_rev <- ff_rev %>%
  ...
  select(-c(ref_02:ref_35, rev_99, rev_NA, rev_76, ref_CHECK#, pension_amt , rev_76,
          #  , eehc
            ))
...
```{r tbl-final-table-with-labels, include = FALSE, eval=FALSE}
...
revenue_change2 %>% 
  kbl(caption = "Yearly Change in Revenue - All Sources", row.names = FALSE) %>% 
   kable_classic() %>%
...
write.xlsx(dataset_names, file = 'summary_file_FY2022_allcats.xlsx')
```
...
Export summary file with Totals. Saves multiple summary tables in one excel file named `summary_file_FY22_wTotals.xlsx`.

```{r export-summary-file, eval=FALSE}
library(openxlsx)
...
```{r summary-file-nototalrow, eval=FALSE}
## Does not have totals in tables!
...
write.xlsx(dataset_names, file = 'summary_file_FY2022_nototals.xlsx')
```

# Dropping COVID State CURE Dollars {#sec-dropped-CURE}
