...
sankeydata <- read_csv("sankeyattempt2022.csv") %>% #exact same as before,
  filter(StFund == "Total")
...
sankey <- sankeydata %>% select(Federal, StateFunds, StFund, Expenditures, stfundname, value, Notes) %>%
  mutate(StateFunds = factor(StateFunds, levels = c("Total_received_fy20", "Total_received_fy21", "Total_received_fy22")),
         ...
         Federal = as.factor(Federal),
                   ...
                   Expenditures = factor(Expenditures, levels = c("Transit", "Medicaid", "Public Health",  "K-12", "Federal Other", "Community Development", "Misc.")),
         Notes = factor(Notes, levels = c(  "CARES", "Other *", "CRRSA", "ARPA")))
...
sankeydata_spent <- read_csv("sankeyattempt2022.csv") %>% 
  filter((StFund == "Total_spent" | StFund == "Allocated" )& Dollars == "Included") %>% 
...
ggplot(sankeydata_spent, 
  ...
  geom_flow(aes(fill=Expenditures),color = "black", reverse=FALSE) +
  # guides(fill = FALSE) +   
  geom_stratum(reverse=FALSE)+
...
sankey <- sankeydata %>% select(Federal, StateFunds, StFund, Expenditures, stfundname, value, Notes) %>%
  ...
  mutate(StateFunds = factor(StateFunds, levels = c("Total_received_fy20", "Total_received_fy21", "Total_received_fy22")),
         ...
         Federal = as.factor(Federal),
                   ...
                   Expenditures = factor(Expenditures, levels = c("Community Development", "Public Health", "Medicaid", "K-12", "Federal Other", "Transit", "Misc.")),
         Notes = factor(Notes, levels = c(  "CARES", "Other *","CRRSA", "ARPA")))
...
~~The graphs below do not include FY23 allocations for federal funds. Dates on top are Fiscal Year received. Dates in the middle of the graph are Fiscal Year expenditures.~~

Remember, federal funds for COVID recovery have been received and spent in different years.

### Fiscal Years 2020-2022

Between FY20 and FY22, \$12.2 billion in State CURE and ESSER funds have been spent by the state. \$1.92

So far, over \$7.629 billion of the State CURE funds (from CARES Act and ARP Act State Fiscal Recovery Fund) have been spent in FY2020-2022.

In FY22, \$4.9 billion (of \$8.127 billion) ARPA-State CURE dollars were spent: \$2.7 billion on repaying unemployment insurance trust fund, \$1 billion transferred to the general revenue fund to make up for any lost revenue caused by the pandemic, and \$1.23 billion on other programs and services (e.g. hospital stability payments, operational expenses, back to business grants and economic development).
...
In FY21, \$1.82 billion from the CARES-State CURE went for operations and grants (business interruptions, child care grants, healthcare providers, rent/mortgage assistance, public health response, etc.), \$569 million CARES-ESSER II funds for K-12 education,
