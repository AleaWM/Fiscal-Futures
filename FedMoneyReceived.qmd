...
Other federal aid came in the form of grants to state departments: ESSER III funds from ARPA paid for \$838 million in K-12 expenditures.

Some CRRSA dollars were also spent in FY22: \$1.1 billion ESSER II and another \$1.5 billion from other federal grants. (e.g. \$332 million from a child care development block grant, 349 million for housing stability, and \$664 million for other public health services like testing and contact tracing).
...
ggplot(cure_exp2022, 
 ...
 #geom_text(stat = "stratum", label.strata = TRUE, reverse=FALSE) + 
  ...
  theme_void() + 
  theme(legend.position="bottom")+
  labs( title = "CURE, ESSER, and other Federal Grants = $15.3 Billion Spent as of end of FY22", subtitle = "Year Spent, Agency that Spent it & FF Spending Category")
...
cure_exp2022 %>% 
  filter(State_local == "State CURE") %>% 
  ggplot(aes(y = `FY Expenditures`, axis3 = `State_local2`, axis2 = Agency, axis1=FF_Cat2, label = "stratum"))+
...
ggplot(cure_exp2022, 
  ...
  theme_void() + 
        geom_text(stat = "stratum", aes(label = after_stat(stratum)), size = 2, reverse=FALSE) +
  theme(legend.position = "bottom")+
  labs(title = "Expenditures: Funds from the State of Illinois or State Departments, 
  Year Spent, & FF Expenditure Purpose", 
...
ggplot(cure_exp2022, 
  ...
  labs( title = "Expenditures So far: Year Received, Federal Fund, Year Spent,
          & How Money was Used",
        caption = "CRF & SLFRF make up the Federal Stimulus Packages, aka State CURE funds.")
```

-   \$500 million spent in FY2020 (CARES State CURE & CARES-ESSER I), \$3.82 billion spent in FY2021 (CARES State CURE, CRRSA-ESSER II,... ) and 11.03 billion spent in FY 2022 (remaining \$0.5 billion CARES-State CURE, \$5.2 billion from ARPA-State CURE, \$2 billion from ESSER II & III, plus other funds from federal grants to state agencies).
...
Graphs below include how money has been spent through FY22 with unspent funds labeled as `FY2023+`. Only State CURE funds are included. ESSER and other federal grants to state departments are excluded.
...
ggplot(aes(y = `FY Expenditures`, 
           axis4 = `Federal Funds`,  axis3 = Agency, 
           axis2 = FY_Spent, axis1=FF_Cat2, label = "stratum")) +
  geom_flow(aes(fill = Law), color = "black", reverse=FALSE) +
  geom_stratum(reverse=FALSE)+  coord_flip()+
   scale_fill_brewer(palette = "YlOrRd", direction = -1)+
...
ggplot(cure_exp2023, 
  ...
  theme_void() + 
      theme(legend.position = "bottom", legend.title = element_blank())+
        ...
        geom_text(stat = "stratum", aes(label = after_stat(stratum)), size = 2, reverse=FALSE) +
  labs(title = "ARPA & CARES State CURE funds, Year Spent/Allocated, and Purpose of Expendtire",
       subtitle = "Purpose of Expenditures using Fiscal Futures Categorization")
