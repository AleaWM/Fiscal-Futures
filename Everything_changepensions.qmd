...
n_year_change = as.numeric(2024-1998)
...
revenue_change2 <- rev_long %>%
  ...
  select(-c( Dollars_1998, Dollars_2024, Dollars_2023, `1 Year CAGR`:`10 Year CAGR`)) %>%
...
rev_long_majorcats <- rev_long |>
  mutate(Category_name = ifelse(Category_name %in% rev_majorcats$Category_name, Category_name, "All Other Expenditures **"),
...
cagr_calculations(exp_long_majorcats)
...
cagr_calculations(rev_long_majorcats)
...
revenue_change_majorcats <- rev_long_majorcats %>%
 ...
 left_join(CAGR_revenues_majorcats_tot, by = c("Category_name" = "Expenditure Category")) %>% 
  arrange(-`FY 2024 ($ Billions)`)%>%
  mutate(`26-Year CAGR` = percent(`26-Year CAGR`/100, accuracy=.1)) %>%
  select(-c(Dollars_1998, Dollars_2024, Dollars_2023, `1 Year CAGR`:`10 Year CAGR`
            )) %>%
...
write.xlsx(dataset_names, file = paste0("data/FY", current_year, " Files/summary_file_FY24_PensionChange_2025_07_07.xlsx"))
```
