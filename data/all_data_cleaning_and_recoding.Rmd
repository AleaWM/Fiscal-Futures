...
agencies_exp <- exp_temp %>% distinct(agency, agency_name) %>% filter(!is.na(agency_name))
...
sources_rev <- rev_1998_2023 %>% distinct(source, source_name) %>% filter(!is.na(source_name))
agencies_rev <- rev_1998_2023 %>% distinct(agency, agency_name) %>% filter(!is.na(agency_name))


# fund info to revenue for all years
rev_1998_2023 <- rev_1998_2023 %>% 
  select(-c(fund_name, agency_name, source_name)) %>%
  #left_join(sources_rev) %>%
  left_join(agencies_rev)
