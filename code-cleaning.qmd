...
agencies_exp <- exp_1998_2023 %>% distinct(agency, agency_name) %>% filter(!is.na(agency_name))
funds_exp <- exp_1998_2023 %>% distinct(fund, fund_name) %>% filter(!is.na(fund_name))
...
funds_rev <- rev_1998_2023 %>% distinct(fund, fund_name) %>% filter(!is.na(fund_name))
...
rev_temp <- inner_join(rev_1998_2023, funds_ab_in_2023, by = "fund") %>% 
  arrange(source) %>% left_join(funds_rev)
